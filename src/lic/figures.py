...
def fig_liability(t: np.ndarray, cf: np.ndarray, duration: float, dest: Path) -> None:
    ...
    ax.set_ylabel("Rentes attendues (M$/an)")
...
def fig_surplus(paths: dict[str, pd.Series], dest: Path) -> None:
    ...
    for (name, s), color in zip(paths.items(), OKABE_ITO, strict=False):
        ax.plot(s.index, s.to_numpy() / 1e6, color=color,
                label=f"{name} : min {_mm(float(s.min()))}, fin {_mm(float(s.iloc[-1]))} M\\$")
    ...
    ax.set_ylabel("Surplus (M$, actif moins passif)")
    ax.yaxis.set_major_formatter(fr)
    ax.legend(fontsize=8.5, loc="best")
    ax.set_title("Les taux clés ramènent le surplus à son point de départ ; la duration seule le laisse errer")
    fig.savefig(dest)
    plt.close(fig)
...
def fig_licat(pertes: dict[str, float], exigence: float, perte_realisee: float,
    ...
    noms = [f"scénario {i}" for i in range(1, 5)]
    ...
    ax.axhline(perte_realisee / 1e6, color=OKABE_ITO[2], linestyle="--", linewidth=1.6,
               label=f"pire perte de surplus réalisée ({_mm(perte_realisee)} M$)")
    ax.axhline(0, color="0.3", linewidth=0.9)
    ax.set_ylabel("Perte de valeur nette sous choc (M$)")
    ax.yaxis.set_major_formatter(fr)
    ax.legend(fontsize=9)
    ax.set_title(f"Exigence TSAV au {date_calcul} : {_mm(exigence)} M$ (pire des quatre scénarios)")
    fig.savefig(dest)
    plt.close(fig)
